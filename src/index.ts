...
( async () => {
    function clickNWait(selector: string) {
        return Promise.all([
    ...
    await page.$eval('input#email', el => (el as HTMLInputElement).value = config.username )
    await page.$eval('input#pass', el => (el as HTMLInputElement).value = config.password )
    ...
    while (true) {
        ...
        try {
            ...
            for (let id of ids) {
                ...
                if (String(id).startsWith('comment')) continue
